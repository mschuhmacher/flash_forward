...
void main() {
  ...
  setUp(() async {
    ...
    // upsertSession() which routes through the same persistence path used
  ...
  group('usagesOfWorkout', () {
    test(
      'matches by direct id (catalog workout dropped into a template)',
      () async {
        final w = _workout(id: 'cat-w', exercises: []);
        await provider.upsertSession(_session(id: 's1', workouts: [w]));
        await provider.upsertSession(_session(id: 's2', workouts: []));
    ...
    test('matches by templateId (deep-copied workout in a template)', () async {
      ...
      await provider.upsertSession(_session(id: 's1', workouts: [embedded]));
    ...
    test('returns empty when no template uses the workout', () async {
      await provider.upsertSession(_session(id: 's1', workouts: []));
  ...
  group('usagesOfExercise', () {
    test('finds nested exercise occurrences across templates', () async {
      ...
      expect(usages.map((u) => u.session?.id).whereType<String>().toSet(), {
        's1',
        's2',
      });
    ...
    test('workoutsContainingExercise dedupes by workout.id when sessions hold '
        'separate Workout instances with the same id', () async {
  ...
  group('propagateWorkoutToSessionTemplates', () {
    test(
      'updates only matching templates and uses independent deep copies',
      () async {
        ...
        await provider.upsertSession(
          _session(id: 's3', workouts: [unrelatedW]),
        );
        ...
        // affect the other.
        expect(
          identical(s1.workouts.single.exercises, s2.workouts.single.exercises),
          isFalse,
        );
        expect(
          identical(
    ...
    test('embedded copies retain the catalog id after propagation', () async {
      ...
      final embeddedA = _workout(
        id: 'cat-w',
        exercises: [ex.deepCopy(keepId: true)],
      );
      final embeddedB = _workout(
        id: 'cat-w',
        exercises: [ex.deepCopy(keepId: true)],
      );
    ...
    test(
      'second-pass: usagesOfWorkout still finds siblings after propagation',
      () async {
  ...
  group('propagateExerciseToSessionTemplates', () {
    test(
      ...
      () async {
        ...
        final w1 = _workout(
          id: 'w1',
          exercises: [targetEx, otherEx],
        ); // direct id
        final w2 = _workout(id: 'w2', exercises: [targetCopy]); // by templateId
        final w3 = _workout(id: 'w3', exercises: [otherEx]); // unrelated

        await provider.upsertSession(_session(id: 's1', workouts: [w1]));
        await provider.upsertSession(_session(id: 's2', workouts: [w2]));
        await provider.upsertSession(_session(id: 's3', workouts: [w3]));
        ...
        // between templates.
        expect(
          identical(s1ws.exercises[0], s2.workouts.single.exercises.single),
          isFalse,
        );
      },
    );
    ...
    test(
      'embedded exercise copies retain the catalog id after propagation',
      () async {
  ...
  group('propagateWorkoutToSessionTemplates with onlyToSessionIds filter', () {
    test('filters to specified sessions, leaves others unchanged', () async {
      ...
      await provider.upsertSession(_session(id: 's-a', workouts: [w]));
      await provider.upsertSession(
        _session(id: 's-b', workouts: [w.deepCopy()]),
      );
      await provider.upsertSession(
        _session(id: 's-c', workouts: [w.deepCopy()]),
      );
  ...
  group('propagateBag with PropagationSelection', () {
    test('empty selection writes nothing', () async {
      ...
      await controller.propagateBag(bag, selection: selection);
    ...
    test('exercise-in-workouts selection (session-scoped edit path) only updates '
