...
Exercise _e({required String id, String? templateId, int sets = 3}) => Exercise(
  id: id,
...
void main() {
  ...
  test(
    '1. workout edit propagates with supersets intact + independence',
    () async {
      ...
      await provider.upsertSession(_s(id: 's1', workouts: [embedded]));
      ...
      expect(
        identical(
          s1.workouts.single.supersets.first,
  ...
  test('4. second-pass propagation preserves supersets', () async {
    ...
    final embeddedA = _w(
      id: 'cat-w',
      exercises: [exA.deepCopy(keepId: true), exB.deepCopy(keepId: true)],
  ...
  test('5. supersetSets survives propagation', () async {
    ...
    final ss = _ss(id: 'ss1', exerciseIds: ['ex-a', 'ex-b'], supersetSets: 5);
    final embedded = _w(id: 'cat-w', exercises: [exA, exB], supersets: [ss]);
    await provider.upsertSession(_s(id: 's1', workouts: [embedded]));
