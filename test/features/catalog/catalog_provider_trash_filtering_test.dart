...
void main() {
  ...
  group(
    'presetWorkouts / presetExercises / presetSessions trash filtering',
    () {
      ...
      test('trashed default workout hidden from presetWorkouts', () {
        catalog.debugSeedDefaults(workouts: [_workout(id: 'def-w')]);
      ...
      test('trashed user exercise hidden from presetExercises', () async {
        await catalog.upsertExercise(_exercise(id: 'e-1'));
      ...
      test('trashed default exercise hidden from presetExercises', () {
        catalog.debugSeedDefaults(exercises: [_exercise(id: 'def-e')]);
      ...
      test('trashed user session hidden from presetSessions', () async {
        await catalog.upsertSession(_session(id: 's-1'));
      ...
      test('trashed default session hidden from presetSessions', () {
        catalog.debugSeedDefaults(sessions: [_session(id: 'def-s')]);
      ...
      test(
        'restoring a trashed item brings it back to presetWorkouts',
        () async {
          await catalog.upsertWorkout(_workout(id: 'w-1', title: 'W'));
          await trash.deleteToTrash(id: 'w-1', kind: TrashKind.workout);
          expect(catalog.presetWorkouts.any((w) => w.id == 'w-1'), isFalse);
      ...
      test('trash kind filtering: trashed workout does not hide exercises', () {
        catalog.debugSeedDefaults(exercises: [_exercise(id: 'e-1')]);
      ...
      test(
        'trashed default workout is removed from presetWorkouts on delete',
        () async {
          catalog.debugSeedDefaults(
