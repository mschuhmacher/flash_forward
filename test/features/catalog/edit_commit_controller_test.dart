...
void main() {
  ...
  late EditCommitController controller;

  setUp(() async {
    TestWidgetsFlutterBinding.ensureInitialized();
    SharedPreferences.setMockInitialValues({});
    tmpDir = await Directory.systemTemp.createTemp('edit_commit_');
    PathProviderPlatform.instance = _FakePathProvider(tmpDir.path);
    provider = CatalogProvider();
    controller = EditCommitController(provider);
  });

  tearDown(() async => tmpDir.delete(recursive: true));

  group('commitChanges', () {
    test('lists sessions affected by changed workouts; suppresses exercise-level '
        'entry when the exercise lives inside a bagged workout', () async {
      ...
      final otherSession = _session(id: 'other-s', workouts: [siblingWorkout]);

      await provider.upsertExercise(ex);
      await provider.upsertWorkout(parentWorkout);
      await provider.upsertWorkout(siblingWorkout);
      await provider.upsertSession(sessionUsingParent);
      await provider.upsertSession(otherSession);
      ...
      final result = await controller.commitChanges(
        bag,
      ...
      expect(result.affectedSessionsByWorkoutId['cat-w']!.map((s) => s.id), [
        'cat-s',
      ]);
      ...
      expect(result.affectedWorkoutsByExerciseId.containsKey('cat-e'), isFalse);
    });
    ...
    test(
      ...
      () async {
        ...
        await provider.upsertExercise(e1);
        await provider.upsertExercise(e2);
        await provider.upsertWorkout(w1);
        await provider.upsertWorkout(otherWorkoutUsingE2);
        // Sessions wrap the workouts so usagesOfExercise can locate them.
        await provider.upsertSession(_session(id: 's1', workouts: [w1]));
        ...
        final result = await controller.commitChanges(bag);
        ...
        expect(result.affectedWorkoutsByExerciseId['e2']!.map((w) => w.id), [
          'w-other',
        ]);
      },
    );
    ...
    test(
      'without excludeWorkoutId, all consuming workouts are listed',
      () async {
        ...
        final bag =
            PendingChangeBag()..addExercise(_exercise(id: 'cat-e', sets: 9));
    ...
    test('excludeSessionId suppresses the session being edited', () async {
      ...
      await provider.upsertWorkout(w);
      await provider.upsertSession(_session(id: 's1', workouts: [w]));
      await provider.upsertSession(_session(id: 's2', workouts: [w]));

      final bag =
          PendingChangeBag()..addWorkout(
            _workout(id: 'cat-w', title: 'Renamed', exercises: []),
          );
    ...
    test('hasAny is false when there are no other consumers', () async {
      ...
      await provider.upsertExercise(_exercise(id: 'lonely-e'));
      await provider.upsertWorkout(_workout(id: 'lonely-w', exercises: []));
    ...
    test(
      'session-scoped commit does not promote workouts to catalog',
      () async {
        final ex = _exercise(id: 'cat-e');
    ...
    test(
      'session-scoped commit does not promote exercises to catalog',
      () async {
        final ex = _exercise(id: 'cat-e');
        final catalogW = _workout(id: 'cat-w', exercises: [ex]);
        final sa = _session(id: 'cat-s', workouts: [catalogW]);
    ...
    test(
      'catalog-scoped commit still promotes workouts (regression guard)',
      () async {
        final w = _workout(id: 'cat-w', exercises: []);
    ...
    test('dedupes affected workouts by id when two sessions reference the same '
        'workout via separate Workout instances', () async {
      ...
      final wA = _workout(id: 'shared-w', title: 'Full-Body', exercises: [ex]);
      final wB = _workout(
        id: 'shared-w',
        title: 'Full-Body',
    ...
    test(
      ...
      () async {
        ...
        final sA = _session(id: 's-a', workouts: [legacyEmbed('uuid-a')]);
        final sB = _session(id: 's-b', workouts: [legacyEmbed('uuid-b')]);
        final sC = _session(id: 's-c', workouts: [legacyEmbed('uuid-c')]);
        ...
        // templateId=cat-w.
        final editedWorkout = legacyEmbed(
          'uuid-a',
        ).copyWith(timeBetweenExercises: 999);
        final bag =
            PendingChangeBag()
  ...
  group('propagateBag', () {
    test('runs exercise→sessions, exercise→workouts, and workout→sessions; '
        'mutating one propagated copy does not bleed into another', () async {
      ...
      final wA = _workout(id: 'wA', exercises: [originalEx]);
      final wB = _workout(
        id: 'wB',
        exercises: [_exercise(id: 'instance-e', templateId: 'cat-e', sets: 3)],
      );

      await provider.upsertExercise(originalEx);
      await provider.upsertWorkout(wA);
      await provider.upsertWorkout(wB);
      ...
      await controller.commitChanges(bag);
      await controller.propagateBag(bag);

      // Both user workouts now carry the new exercise sets.
      final wAAfter = provider.presetWorkouts.firstWhere((w) => w.id == 'wA');
      final wBAfter = provider.presetWorkouts.firstWhere((w) => w.id == 'wB');
    ...
    test('session changes do not trigger any propagation', () async {
      ...
      await provider.upsertExercise(ex);
      await provider.upsertWorkout(w);
      await provider.upsertSession(_session(id: 'cat-s', workouts: [w]));

      final bag =
          PendingChangeBag()..setSession(
            _session(id: 'cat-s', title: 'Renamed', workouts: [w]),
          );

      // Snapshot user lists pre-propagate.
      final wBefore = provider.presetWorkouts.firstWhere(
        (x) => x.id == 'cat-w',
      );
      final eBefore = provider.presetExercises.firstWhere(
        (x) => x.id == 'cat-e',
      );
  ...
  group('propagateExerciseToWorkouts', () {
    test('replaces matching exercises in user workouts (by id and templateId) '
        'and gives each workout an independent deep copy', () async {
      ...
      final w3 = _workout(id: 'w3', exercises: [_exercise(id: 'unrelated-e')]);

      await provider.upsertWorkout(w1);
      await provider.upsertWorkout(w2);
      await provider.upsertWorkout(w3);
    ...
    test(
      'embedded exercise copies in workouts retain the catalog id after propagation',
      () async {
        ...
        for (final workout in provider.presetWorkouts) {
          for (final e in workout.exercises) {
            expect(
              e.id,
              'cat-e',
              reason: 'id must remain catalog id after propagation',
            );
          }
        }
      },
